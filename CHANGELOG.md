...
- **The interactive protein network no longer draws proteins on top of each other.** Its force-directed layout ran in draft quality and ignored node and label size, so cliques and two-protein components collapsed into stacks; on the pasilla network 433 pairs of nodes or labels overlapped, involving 58 of its 68 proteins. The layout now spaces nodes by their drawn size and labels. Measured in the application's own renderer, no pair overlaps on the pasilla, UME6 and Arabidopsis networks, and the dense 300-node yeast cbc2 network drops from 1,378 overlapping pairs to 17; laying it out takes 0.8 s instead of 0.2 s. Because the whole network now spreads across the view, protein labels appear once you zoom in far enough for them to be legible, rather than stacked at the first view; hovering a protein still names it. The static network figure is unchanged.
- **Hand-set CPU and memory limits are saved as the custom profile.** Changing the manual limits left the profile at its previous name, so a project could record `profile: balanced` with 24 threads and 58 GB, and a later runtime estimate reset the hand-set values to the named profile's recommendation. Editing either limit now selects custom; values the application detects, recommends or loads keep the named profile.
- **Programs the application opens no longer load its bundled libraries.** Files, folders and reports were opened with the frozen application's DLL search directory still set, and a program started that way inherits it: a browser opened on a report ran helper processes that loaded the bundled `VCRUNTIME140.dll`, which kept the install folder in use until the browser closed. They now open with the standard search path, restored for the application afterwards.
- **Updating over an existing installation removes the old folder reliably.** Setup deleted the old folder once, straight after the uninstaller; a file still locked for a moment left the folder behind, and the wizard then asked whether to install into an existing folder. It now retries for up to 15 s. A reinstall with a file held open for 8 s showed the prompt before this change and does not after it. The update also offers the folder the previous version was installed in; removing the old version erased that record, so an installation in a folder of your choosing moved to the default folder on update.
- **The space-reclaim script recognises finished microarray and imported-results runs.** `scripts/reclaim_run_space.sh` required a count table, which those routes never write, so it refused a completed run with "this run did not produce its key outputs". It now reads the input type from the configuration, as the workflow does, and requires the count table only on read-based routes.
...
- **`bulkseq --help` lists every exit status,** generated from the constants the command returns.
- **The documentation separates warnings from cautions.** Every callout states its kind in text: a warning where ignoring it can silently invalidate a result or lose data, a caution where the failure is recoverable and noticed, a check for what to confirm before continuing, a note for context, and a release notice. The 51 undifferentiated boxes became 17 warnings, 15 cautions, 27 checks, 5 notes and 9 release notices, with boxes that mixed kinds split. The site build refuses a callout without a kind.
- **A new reference page, Exit codes and statuses, and a matching README section** give every check status, including the interface-only `STALE`, with every command-line exit code, environment-setup code, run-monitor state and public-data lookup message, and what to do for each. They state that `bulkseq run` does not evaluate the pre-run checks, so a script should call `bulkseq check` first and stop on exit 4.
- **The checks page lists what each of the 22 check files tests,** which statuses each can emit, and which can stop a running workflow: a `FAIL` in check 00, anything but `PASS` in check 05 on the FASTQ and SRA routes, and a failed microarray import recorded in checks 11 and 12.
- **The Linux build now measures the AppImage's glibc floor** from its bundled libraries and fails when the README or the install guide states a different one, so the documented requirement cannot drift when the build runner changes.
...
- Known issue, documented rather than changed in this release: on the two bundled *Fusarium graminearum* datasets, enrichment ends REVIEW_REQUIRED. The KEGG identity check compares organism names exactly, so it rejects code `fgr`, which KEGG names *Fusarium graminearum*, for the configured *Fusarium graminearum* PH-1 although both carry taxon 5518; KEGG enrichment is then reported as not interpretable. g:Profiler also returns no GO terms for the 5,201 significant genes, which points to an identifier mismatch. The behaviour is the same in the 0.30.1 runs of these data. Fixing it changes enrichment output, so it is deferred to a minor release.
...
- The README and the tutorial list the result of every bundled dataset under 0.32.1, and say which reproduce a recorded value and which are recorded for the first time.
