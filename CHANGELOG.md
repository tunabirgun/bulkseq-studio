...
- **Faster, verified FASTQ downloads.** SRA/ENA downloads now use **aria2** with 16 parallel connections per file when it is available (falling back to a single stream otherwise), which is dramatically faster than one throttled connection (measured ~24 MiB/s vs a fraction of that before). Each file is verified against ENA's published **MD5** after download, so a truncated or corrupted transfer is caught rather than silently used — the checksum result is reported in the results report (a data-integrity guarantee). aria2 was added to the pipeline environments.
...
- **Pipeline environments** gained the new tools: `aria2`, `edger`, `trim-galore`, `cutadapt`, `pigz`, `trimmomatic`, `fastq-screen`, `bowtie2`, `ribodetector`, `gsva`, `rseqc`, and the UCSC `gtfToGenePred`/`genePredToBed` utilities. The pinned lock (`bulkseq.lock.yaml`) was regenerated; RiboDetector installs its CPU ONNX runtime (no CUDA).
