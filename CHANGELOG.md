...
- **Windows installer** detects an existing BulkSeq Studio install and offers to update (remove the old version and install this one) or uninstall before continuing, instead of installing over the top. The setup wizard is branded with the BulkSeq Studio logo in place of the default Inno Setup artwork.
