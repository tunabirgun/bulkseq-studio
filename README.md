...
*The full analysis flow. Two read inputs and three shortcut entry points feed four stages (QC & trimming → alignment & quantification → differential expression → downstream outputs), with an optional multi-study meta-analysis lane. Inner-bar groups (fastp / Trim Galore / Trimmomatic; STAR / HISAT2 / Salmon; DESeq2 / limma-voom / edgeR) are pick-one; dashed boxes are optional.*
...
- **Choice of aligner.** STAR (default, genome-size-aware index), HISAT2 (graph aligner, small index and low memory, for large crop genomes that overflow STAR), or Salmon (mapping-based transcriptome quantification, no BAM files, lowest memory). The three routes produce a common count matrix, so everything after counting is the same. See [Choosing an aligner](#choosing-an-aligner).
...
- **Start from a count matrix.** Already have counts? Upload a gene-by-sample table (featureCounts output or any TSV/CSV) to skip download, QC, and alignment and go straight to DESeq2, figures, and enrichment. The table must hold raw integer counts: normalized values (TPM/FPKM, log-CPM, or microarray RMA intensities) are refused on import because the DESeq2 count model needs raw counts, while RSEM/tximport estimated (non-integer) counts can be opted in.
...
- **Publication figures.** PCA, sample-distance, MA, volcano, a combined top-DEG heatmap, separate **up-** and **down-regulated** top-DEG heatmaps, a raw p-value histogram, and (RNA-seq) dispersion / Cook's-distance / library-size diagnostics, each exported as PNG (raster) and SVG (vector, with an in-app preview toggle). A built-in **Figure Style** editor (palette, fonts, sizes, point size, label counts, dimensions in in/cm/px, DPI, a volcano y-axis scale mode, and per-sample label visibility) re-renders figures with **Regenerate figures**, without re-running alignment or DESeq2. The volcano y-axis has three modes — *cap the tail* (default: squish hyper-significant genes to a cap line with off-scale triangle markers), *show all at full height* (every gene at its true −log10 adjusted p, so a marginal gene with an extreme p-value is visible), and *compress (sqrt)* — so an outlier gene need not squash the rest of the plot.
...
The three aligner routes (STAR, HISAT2, Salmon), the featureCounts / STAR-gene-counts / Salmon-tximport quantifiers, the three trimmers (fastp / Trim Galore / Trimmomatic), the two rRNA tools (SortMeRNA / RiboDetector) and the optional FastQ Screen contamination report, the three differential-expression engines (DESeq2 / limma-voom / edgeR), the multi-study meta-analysis, single-end and paired-end input, GSVA pathway activity, RSeQC extended QC, custom gene-set enrichment, and the count-matrix, GEO microarray (limma), and DESeq2-results input routes are all implemented; the routes carrying a benchmark family are listed under [Validation](#validation), and the remaining options were confirmed to run to completion on real reads.

## Choosing an aligner

All three aligners feed a gene-level count matrix in the same format, so the whole downstream (differential expression, enrichment, the PPI network, every figure) runs the same way whichever you pick. The aligners count reads by different methods, so the exact numbers differ slightly (they are highly concordant, not bit-identical; on the rice benchmark Salmon called 12,609 DE genes against STAR's 12,171). The choice is mainly about speed, memory, and whether you want alignment files (BAMs). On STAR you can choose the counter: featureCounts (default) or STAR's own gene counts, which need no extra pass. HISAT2 is fixed to featureCounts and Salmon to tximport, so the combo locks itself to the one valid choice.

| Aligner | Counter | Memory | BAM files? | Choose it when |
|---|---|---|---|---|
...
On Linux, BulkSeq Studio runs **natively** — there is no WSL. The same PySide6 GUI drives the same Snakemake pipeline directly in a local [micromamba](https://mamba.readthedocs.io/) environment. On Windows the pipeline itself executes inside WSL2, so both platforms run the same Linux binaries and only the launcher differs. The bundled pasilla benchmark gives the 467 differentially expressed genes reported under [Validation](#validation) under the shipped default, which keeps organellar genes. Repeating the same pasilla count matrix is bit-identical across reruns, Snakemake core counts and BLAS thread settings — 174 output comparisons over seven runs, none differing — measured on one machine.
...
> **Resuming an interrupted run.** You can stop a run, close BulkSeq Studio, and
> continue later. Reopen the project and the Run Monitor shows a **Resume
> Interrupted Run** banner that unlocks and continues from where it left off,
> reusing every completed step — a full rebuild is not triggered. The always-present
> **Resume** and **Unlock** buttons do the same manually.
...
`bulkseq` scripts the same pipeline without the GUI. Both front ends build their Snakemake
invocation with the same function, so a CLI run and a GUI run of one project issue an
identical command; a test asserts it.

```bash
bulkseq project create --name my_study --workdir ~/projects
bulkseq config set deseq2.alpha 0.01     # validated; an out-of-range value is refused
bulkseq check                            # sample sheet and design
bulkseq print-command --mode dry-run     # see the exact command before running it
```

![Configuration is validated on write, not just stored](docs/assets/cli/cli-config.svg)

### On a cluster

`--exec-profile` selects where jobs run. The pipeline does not change.

| Target | Behaviour |
| --- | --- |
| `local` *(default)* | This machine — WSL2 on Windows, native on Linux. |
| `slurm` | One `sbatch` per job; each rule's declared resources become its allocation. |
| `kubernetes` | One pod per job. Requires a container image (below). |

Site settings live in `workflow/profiles/site/<target>/config.yaml`. Slurm needs at least a
partition:

```yaml
default-resources:
  - runtime=240
  - slurm_partition=short      # `sinfo -s`, or ask your support desk
```

Two things to know before a first cluster run. **Compute nodes often have no outbound
internet** — download ENA/SRA FASTQ files on the login node first. And **Kubernetes needs a
container image**: no rule declares a `conda:` directive, so a pod starts from an image that
must already carry the `bulkseq.lock.yaml` tool stack. Set `container-image` in that profile.
A `--dry-run` passes without pulling an image, so it validates the graph and nothing else.

Full profile reference and per-site notes: [documentation](https://tunabirgun.github.io/bulkseq-studio/).

> The cluster paths are verified off-cluster — generated commands, profile resolution and
> resource handling are covered by tests, and `--dry-run` succeeds against each executor.
> No submission to a real scheduler has been made. Take the command from
> `bulkseq print-command --exec-profile slurm` and run the bundled pasilla benchmark through
> it before committing a study to a new cluster.

## Validation

The pipeline is validated end-to-end on real datasets. **The KEGG background correction, measured.** `enrichKEGG` was changed to pass the tested-gene background, matching `enrichGO`/`enrichDO` (see [Features](#features)). Scoring the same differentially expressed gene sets both ways gives:

| Dataset | Genes in the KEGG background | DE genes scored | KEGG terms, tested-gene background | KEGG terms, genome background | Lost | Gained |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| *Drosophila*, pasilla (STAR) | 7,386 | 142 | 0 | 0 | 0 | 0 |
| *Fusarium graminearum* (STAR) | 8,280 | 3,251 | 0 | 4 | 4 | 0 |
| *Oryza sativa* (STAR) | 23,935 | 4,724 | 19 | 17 | 1 | 3 |
| *Oryza sativa* (Salmon) | 24,316 | 5,086 | 20 | 22 | 3 | 1 |

The scored gene set is the one the pipeline submits to enrichment: its own up- and down-regulated lists, which threshold on adjusted p-value **and** raw fold change (`padj < 0.05`, `|log2FC| >= 1`), not on adjusted p-value alone. For pasilla the background is the 7,386 of 7,532 tested genes that map to an Entrez id, and 142 of the 146 submitted genes fall within that mappable set; the other runs pass their own gene ids to KEGG directly, so no mapping step applies.

The direction depends on the dataset. The correction removed all four *Fusarium* pathways, left pasilla with none under either background, added two net on the rice STAR route and removed two net on the rice Salmon route. It also shrinks the tested family (62→61, 121→113, 134→132 and 136→133 pathways), because restricting the background drops gene-set members that were never expressed and some sets fall below the minimum size. How much of each change comes from that smaller multiple-testing family and how much from the changed hypergeometric background was not decomposed. Eight pathways significant under the genome background are absent from the corrected lists: all four *Fusarium* terms, one rice STAR term and three rice Salmon terms. Scored this way the measurement reproduces the pipeline's own output exactly — 19 terms for the rice STAR run, and the four *Fusarium* terms term-for-term under the pre-correction background that run predates. KEGG GSEA (`gseKEGG`) takes a ranked gene list with no universe argument, so it is unaffected, as are the DE gene counts, GO enrichment, and PPI network numbers throughout this section.

- **Drosophila, pasilla** (Ensembl): 467 DE genes, the *pasilla* gene strongly down-regulated (log2FC −2.00, padj 2.7e−40), all sanity checks PASS. The four-sample subset ships as a one-click benchmark project (`examples/benchmarks/pasilla_paired_subset`). That count is for the shipped default, which **keeps** organellar genes. Setting `organellar_genes: separate` removes 9 mitochondrial genes from the 7,532 tested and gives 473 — the configuration the manuscript's B4/B5 benchmarks use. None of the 9 is itself significant; they shift the result through the size factors and the multiple-testing denominator. Any DE count from this pipeline is specific to its organellar setting.
...
- **Oryza sativa (rice), salt stress** (PRJDB38133; super-hybrid CY1000, IRGSP-1.0 NCBI RefSeq): 12,171 DE genes (5,732 up / 6,439 down at padj < 0.05) on the six-sample control vs 5-day salt subset; KEGG GSEA, g:Profiler GO enrichment, and a 58-node, 76-edge STRING PPI network reproduce the canonical salt-stress response (ROS detoxification and glutathione metabolism, ABA and plant-hormone signalling, ion and amino-acid transport; photosynthesis and primary carbon metabolism down-regulated). The KEGG ORA pathway list for this STAR run is now **re-validated**: its DESeq2 table had been cleaned up after the run, so the whole rice STAR route was re-run from the accessions rather than re-scored. The re-run reproduced the original exactly — the same 23,935 tested genes, the same 12,171 significant, and a maximum absolute log2 fold-change difference of zero across every gene — and its corrected KEGG list appears in the table above beside the Salmon route's. The Salmon route was regenerated the same way and reproduced its deposited table exactly: the same 24,316 tested genes, the same 12,609 differentially expressed, and the same 56-node, 71-edge STRING network. The DE gene counts, GSEA, GO, and PPI results are unaffected by the KEGG correction either way.

The archived pasilla table reproduces to zero difference eight minor releases later, and the Fusarium baseline returns the same gene counts, confirming the analysis is stable across pipeline revisions.

The two alternative aligners are validated against STAR on these datasets. HISAT2 runs pasilla (unstranded) and the reverse-stranded Fusarium set (strandedness auto-detected as 2; 5,812 DE genes, concordant with STAR's 5,836). Salmon (1.10.3) runs pasilla (24,278 genes, 477 DE genes) and the rice crop benchmark (33,844 genes, 12,609 DE genes, a 56-node PPI). Each route reaches the same enrichment and network outputs. The KEGG `osa` ORA figure originally reported alongside this Salmon run has been re-scored under the corrected background: 20 significant pathways instead of 22, keeping 19 of the original 22 (see the table above).

The two alternative differential-expression engines are validated against DESeq2 on the same counts. On *Fusarium graminearum* (spore vs mycelium), limma-voom reaches a log2 fold-change Spearman correlation of 0.997 and a differentially expressed gene Jaccard index of 0.94 against DESeq2, and edgeR quasi-likelihood reaches 1.000 and 0.97, both with 100% agreement on the direction of the shared differentially expressed genes. DESeq2 remains the default, and its result on the *Fusarium* set reproduces the deposited baseline exactly (2,723 up / 2,478 down at |log2FC| > 1), so the alternative engines add a cross-check without perturbing the validated default. The trimmers and rRNA tools were confirmed to run to completion on real reads (including a human airway subset), each producing valid trimmed / filtered output.
...
**Benchmark archive (Zenodo).** The full quantitative validation suite behind the BulkSeq Studio paper (correctness against simulated ground truth, false-discovery and null calibration, accuracy curves, concordance with [nf-core/rnaseq](https://nf-co.re/rnaseq), cross-aligner agreement, input-mode and microarray-route equivalence, multi-organism breadth, runtime/memory scaling, and determinism, plus STAR gene-counts quantifier concordance, custom gene-set enrichment, ribosomal RNA filtering, differential-expression engine concordance, and human-dataset applicability, plus re-validation on the current release, enrichment negative controls, and the null-calibration decomposition — families B1–B18) is archived on Zenodo, together with the simulation and scoring code, the pinned scoring environment, the per-benchmark result tables and figures, and a step-by-step reproduction guide, together with an inventory of which families can be recomputed from the deposited code: **DOI [10.5281/zenodo.21807799](https://doi.org/10.5281/zenodo.21807799)** (version 0.24.0; the concept DOI [10.5281/zenodo.20955660](https://doi.org/10.5281/zenodo.20955660) always resolves to the latest version).
...
(per-study fold changes), a **`compareCluster` enrichment dotplot** that puts each study's up/down sets
and the convergent sets side by side to show shared versus study-specific biology, and — when at least
three studies are combined — a **between-study heterogeneity plot** (pooled effect versus I²), plus a
**combined p-value histogram** and an **integration-gain** bar (genes recovered by pooling versus those
called in a single study). The convergent-gene
